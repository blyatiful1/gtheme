...
- [x] M23 PKGBUILD-git variant for checkout builds; README Arch route corrected (tag cut = deferred) — fixed@465cdcd (PKGBUILD-git); README Arch route text is 4f9f9d0 but packaging flagged it may be stale — re-check `makepkg -p PKGBUILD-git -si` / `pacman -R gtheme-git` wording against README before merge. **Re-check done in the release-prep pass (RP7 below):** the README route is right and `PKGBUILD-git`'s header was the stale half — it claimed to be the file the README's `git clone` route means, which is the plain `PKGBUILD`. Header rewritten and the invocation spelled `makepkg -si -p PKGBUILD-git` in both files; `pacman -R gtheme-git` matches the `pkgname`/`provides`/`conflicts` block and needed no change.
...
## Release-prep review pass — 2026-08-28 (after the `origin/main` merge `7807eeb`, before the tag)

A release-prep read of the tree that is about to be tagged found **eight must-fix items**: one packaging defect that broke the whole Arch install route (RP1), and seven false statements in shipped text. Their provenance, checked against the merge's two parents rather than assumed: RP3 is a regression — `e66183c` had corrected that sentence on this branch and the merge `7807eeb` took `origin/main`'s version of the file, qualifier and all; RP8 is an upstream sentence the merge brought in, never true of the install path this project prescribes; RP2, RP4, RP5 and RP7 were made false by this campaign's own later work (the Windows page's sections, the CI restructure, the H6 two-path correction, the `PKGBUILD-git` M23 re-check) and nobody re-read the prose afterwards. Seven fixed at `b9883d8`; RP6 lives in `.audit/RELEASE_NOTES.md`, which `.git/info/exclude` keeps untracked, so that one is fixed on disk only and is not in any commit. `./verify.sh` green afterwards: **2579 passed, 2 skipped, 29 deselected, ruff clean, `verify.sh: OK`** (matches the campaign's closing gate; this pass touched no test and no source module).

| ID | file | what was wrong | fix, and how it was verified |
|---|---|---|---|
| RP1 | `PKGBUILD`, `PKGBUILD-git` | `check()` ran pytest in a tree where gtheme is **not** installed — build() only builds a wheel — and nothing puts `src/` on `sys.path` (no root `conftest.py`, no `pythonpath` in `[tool.pytest.ini_options]`). Every test module died at collection with `ModuleNotFoundError: No module named 'gtheme'`, `makepkg` aborted at check(), and `makepkg -si` therefore never installed anything. This falsified the README's Arch route, `RELEASE_NOTES.md`'s Install section and `docs/testing.md`. | `PYTHONPATH="$PWD/src"` (`"$startdir/src"` in the -git file) in front of the `dbus-run-session` line. Reproduced in a `git archive HEAD` tree run by an interpreter with no gtheme installed: **43 errors during collection, Interrupted** → after the fix **1918 passed, 2 skipped, 690 deselected**, identical to the editable-venv baseline. Both real `check()` functions were then sourced and run against the finished tree, as were `build()` and `package()`. The environment variable rather than pytest's `pythonpath` ini setting, deliberately: the ini setting fixes collection only, and seven tests re-import gtheme in a **fresh child interpreter** — the two dconf-tier backend probes, the ego client's no-desktop-libraries import, both core-without-GTK imports, and the crash-hook test — which inherits the variable but not pytest's in-process `sys.path`. Measured: with the ini setting alone, `43 errors` becomes `7 failed, 1903 passed`. Installing the built wheel into a temp prefix was tried first and rejected: 32 failures, because the descriptor-corpus tests resolve the checkout's own `data/`. |
| RP2 | `README.md` | Windows & Desktops was described in its pre-fix form — "in two collapsed groups" — which is exactly the arrangement `windows.py` records as *not enough* (persona-report §3.2). The alt text carried it too. | Rewritten to what the page renders: named sections folded open one at a time, a filter box above each set, and the Ctrl+F line. Counts checked against the shipped corpus rather than the prose: `shortcuts` 123 rows in 8 sections, `mediakeys` 52 rows in 6 (the "Everything else" catch-all is empty and dropped) — 175 total, so the README's number was right and only its description was stale. No section count is stated in the README, because that one is corpus-dependent. |
| RP3 | `README.md` | The campaign's "*Before gtheme* if it is there" correction (`e66183c`) was clobbered by the merge, and `origin/main`'s rewritten safety bullet promised the row unconditionally as well — so two sentences promised it while the Safety section three hundred lines below still explained the exception — internally contradictory as well as false: `capture_pristine_point` returns `None` when `already_touched(root)`, and `ensure_pristine_point` logs-and-drops a capture failure. | Both restored: the recovery step reads "including *Before gtheme* if it is there", and the safety bullet now carries the exception and points at the full rule. |
| RP4 | `docs/testing.md` | Four statements about CI were false against the merged `ci.yml`, which is one packaging job plus one container job, not two suite-running jobs: the two "both CI jobs" tier rows, "Every tier except sandbox is run … by both CI jobs and by the Arch package's `check()`" (false twice over — `check()` runs `-m "not gtk and not sandbox"`), "so both jobs run it", and two paragraphs describing a "CI unit job" that no longer exists. | Rewritten against `ci.yml`: the Ubuntu job is `ruff` plus three named packaging files with no PyGObject and no bus; the Arch container job runs every tier; `check()` runs everything but `gtk` and `sandbox`, and why. The tier table now names `verify.sh` and the packaged `check()` per row. |
| RP5 | `CHANGELOG.md` | The last surviving instance of the H6 add-on overclaim: "Installing goes through GNOME's own confirmation box; gtheme never installs one behind it" — false for the Look path, which calls `installer.install_package(...)` (`looks.py:1017`) and has no desktop dialog. README, SECURITY.md and docs/preset-format.md already carried the two-path wording. | Replaced with the same two-path wording those three use: Add-ons page = the desktop's own box; a Look's add-ons = fetched by gtheme, no box, the preview naming every add-on first. |
| RP6 | `.audit/RELEASE_NOTES.md` | "**Removing it**: `./install.sh --uninstall`, which puts back the settings gtheme owns" — the uninstall branch restores nothing; it *refuses* while `current.json`/`ownership.json` are non-empty, or with `--force` removes only the launcher. The same document says so correctly in its Fixed section. | Rewritten to match `install.sh` and the README's Removing section: put the desktop back first, then uninstall takes back the five things outside its folder and refuses while gtheme still owns settings. (This file is untracked — `.git/info/exclude` holds `.audit/` — so the fix is on disk for whoever writes the release body, not in a commit.) |
| RP7 | `PKGBUILD-git` | The header claimed to be "the file the README's `git clone …` route means". It is not: that route is `git clone … && makepkg -si`, the plain `PKGBUILD`, deliberately and as described there. The two files also disagreed on invocation order. This is the re-check M23 asked for before merge. | Header rewritten as "the second of the README's two Arch recipes", and the invocation spelled `makepkg -si -p PKGBUILD-git` to match the README. |
| RP8 | `README.md` | "**You do not need:** … an internet connection (except to browse for new add-ons)" — false for the install route the next section prescribes. `install.sh:243` runs `pip install --quiet -e "$REPO"` with build isolation on, so pip fetches hatchling from PyPI every time; the script's own lines say "getting the one small piece gtheme needs from the internet" and blame the connection when it fails. | Split into what you do not need and the two moments you must be online, plus a "Be online for this step" note at install step 4. Verified rather than reasoned: `pip install --no-index -e <tree>` fails with `ERROR: Could not find a version that satisfies the requirement hatchling`. |

**Not changed, on purpose:** `pyproject.toml` gains no `pythonpath` setting — it would half-fix a bare `pytest` in an uninstalled checkout (collection green, seven fresh-interpreter tests red), which is a worse failure than the current loud one, and the documented developer route (`uv venv --system-site-packages .venv && uv pip install -e '.[dev]'`) is unaffected either way.
