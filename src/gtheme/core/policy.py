...
opens this" keys, the desktop session's own definition, and any raw settings
location outside the add-on trees a decorative Look legitimately reaches into.
A refusal is deliberately not a "skip this part": a Look that needs one of
these is not a decorative Look, and applying the rest of it would be applying
something the author did not design.
...
def _relative(dest: str, root: str | Path | None) -> PurePosixPath | None:
    ...
    try:
        ...
        return PurePosixPath(resolved.relative_to(base.resolve()))
